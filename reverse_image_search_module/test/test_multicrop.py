...
def is_in_reference_many(image_names):
    ...
    for _im in image_names:
        ...
        for x_ini, y_ini, x_end, y_end in gen_multi_cropping(
            image.width, image.height, K_CROPPING
        ):
        ...
        if file_name == _im:
            ...
            positive.append(0)
            mylogger.info(
                f"Not a match in {K_CROPPING} <original>{_im} <distance>{selected_dist}"
            )
...
def test_random_cropp_many_to_many_images():
    ...
    mylogger.info(
        f"MultiCropping: many to many: ACCURACY = {acc} K = {K_CROPPING} N = {N_SEARCH}"
    )
...
def test_random_cropp_one_to_many_distance9_images():
    ...
    mylogger.info(
        f"MultiCropping: one to many distance 0.9 : ACCURACY = {acc} N = {N_SEARCH}"
    )
